         ...
         try
         {
            ...
            using (Microsoft.Win32.RegistryKey registryKey = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(speechTokens))
            {
               if (registryKey != null)
               {
                  ...
                  {
                     string finalKey = "HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Speech\\Voices\\Tokens\\" +
                                       regKeyFound + "\\Attributes";

                     var voiceName = (string)Microsoft.Win32.Registry.GetValue(finalKey, "name", "");

                     if (!string.IsNullOrEmpty(voiceName))
                     {
                        //desc = voice.GetDescription();
                        var desc = voiceName;
                        var gender = (string)Microsoft.Win32.Registry.GetValue(finalKey, "gender", "");
                        var age = (string)Microsoft.Win32.Registry.GetValue(finalKey, "age", "");
                        var lang = (string)Microsoft.Win32.Registry.GetValue(finalKey, "language", "");
                        var vendor = (string)Microsoft.Win32.Registry.GetValue(finalKey, "vendor", "");
                        var version = (string)Microsoft.Win32.Registry.GetValue(finalKey, "version", "");
